                ...
                return InkWell(
                  onTap: () async {
                    ...
                    List<AlbumTrack>? _albumTracks =
                        await fetchPlaylistTracks(_playlist?[i + start].id);

                    /* Stored queue for front page categories */
                    // StaticStore.myQueueTrack = _albumTracks!;
                    Navigator.of(context).push(PageRouteBuilder(
                      pageBuilder: (context, animation, secondaryAnimation) =>
                          AlbumView(_playlist?[i + start].imgUrl,
                              _playlist?[i + start].name, _albumTracks),
                      transitionsBuilder:
                      ...
                      transitionDuration: const Duration(milliseconds: 400),
                        ...
                        // MaterialPageRoute(builder: (context)=>AlbumView(_playlist?[i+start].imgUrl, _playlist?[i+start].name, _albumTracks))
                        );
